...
import { Plus, Trash2, Save, CheckCircle, AlertCircle, ChevronLeft, Upload, X, ImageIcon } from 'lucide-react'
...
export default function AdminCreatePage() {
  ...
  function setList(key: 'highlights' | 'includes', i: number, val: string) {
    setForm(f => { const a = [...f[key]]; a[i] = val; return { ...f, [key]: a } })
  }
  function addList(key: 'highlights' | 'includes') { setForm(f => ({ ...f, [key]: [...f[key], ''] })) }
  function removeList(key: 'highlights' | 'includes', i: number) { setForm(f => ({ ...f, [key]: f[key].filter((_, idx) => idx !== i) })) }
    ...
    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-8">
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <Row label="Trip Image">
            <div className="space-y-2">
              <div className="flex gap-2">
                <Input value={form.image} onChange={v => { set('image', v); setUploadErr('') }} placeholder="https://images.unsplash.com/… or upload →" />
                <button
                  type="button"
              ...
              {uploadErr && (
                <p className="text-xs text-red-500 flex items-center gap-1"><AlertCircle size={12} />{uploadErr}</p>
              )}
              {form.image && !uploading && (
                <div className="relative inline-block">
                  ...
                  <div className="mt-1 text-xs text-gray-400 break-all max-w-xs">
                    <span className="font-medium text-gray-600">URL: </span>{form.image}
                  </div>
                </div>
              )}
              {!form.image && !uploading && (
                <button type="button" onClick={() => fileInputRef.current?.click()}
                  className="flex items-center gap-2 w-full justify-center py-6 rounded-xl border-2 border-dashed text-gray-300 hover:text-gray-400 hover:border-gray-300 transition-colors"
        ...
        <Section title="Pricing & Seats">
          <Row label="Price (₹) *"><Input required type="number" value={form.price} onChange={v => set('price', v)} placeholder="6499" /></Row>
          <Row label="Original Price (₹)"><Input type="number" value={form.originalPrice} onChange={v => set('originalPrice', v)} placeholder="Optional" /></Row>
        ...
        <Section title="What's Included">
          {form.includes.map((inc, i) => (
            <div key={i} className="flex gap-2 mb-2">
              <input value={inc} onChange={e => setList('includes', i, e.target.value)} placeholder={`Item ${i+1}`}
                className="flex-1 text-sm px-3 py-2 rounded-lg border outline-none" style={{ borderColor: '#e5e7eb' }} />
              <button type="button" onClick={() => removeList('includes', i)} className="text-gray-300 hover:text-red-400"><Trash2 size={14} /></button>
            </div>
          ))}
          <button type="button" onClick={() => addList('includes')} className="text-xs flex items-center gap-1 mt-1" style={{ color: 'var(--primary)' }}>
        ...
        <Section title="Departure Batches">
          ...
          <button type="button" onClick={addBatch} className="text-xs flex items-center gap-1" style={{ color: 'var(--cyan)' }}>
