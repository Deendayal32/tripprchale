...
export default function AdminEditPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/admin/trips/${id}`)
      .then(r => r.json())
      .then(data => {
        if (data.error) { setLoadErr(data.error); setLoading(false); return }
        const t = data.trip
        const openMap: Record<number, boolean> = {}
        ;(t.itinerary ?? []).forEach((_: unknown, i: number) => { openMap[i] = false })
        if (openMap[0] !== undefined) openMap[0] = true
        setOpenDays(openMap)
        setForm({
          name:               t.name         ?? '',
          ...
          totalSeats:         String(t.totalSeats  ?? 20),
          seatsLeft:          String(t.seatsLeft   ?? 20),
          image:              t.image        ?? '',
          images:             Array.isArray(t.images) ? t.images : (typeof t.images === 'string' ? JSON.parse(t.images) : []),
          emoji:              t.emoji        ?? '✈️',
          badge:              t.badge        ?? '',
          ...
          batches:            (t.batches?.length     ? t.batches     : [{ departureDate: '', seatsLeft: 20, status: 'Available' }]),
          quadPrice:          t.quad_price   != null ? String(t.quad_price)   : '',
      ...
      .catch(() => { setLoadErr('Failed to load trip'); setLoading(false) })
  }, [id])
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res  = await fetch(`/api/admin/trips/${id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) })
      const data = await res.json()
      if (!res.ok) throw new Error(data.error || 'Failed')
      setStatus('success'); setMessage(`Trip "${form.name}" updated successfully!`)
    } catch (err: unknown) {
  ...
  if (loading) {
    return (
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-8 flex items-center justify-center min-h-[300px]">
  ...
  if (loadErr) {
    return (
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-8">
        <div className="flex items-center gap-2 p-4 rounded-xl text-sm text-red-700" style={{ background: '#fff1f2', border: '1px solid #fecaca' }}>
          <AlertCircle size={16} /> {loadErr}
        </div>
        <Link href="/admin" className="mt-4 inline-flex items-center gap-1 text-sm text-gray-400 hover:text-gray-600">
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-8">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="font-display font-bold text-2xl" style={{ color: 'var(--navy)' }}>Edit Trip</h1>
          <p className="text-gray-400 text-sm">ID #{id} · {form.name}</p>
        </div>
      </div>

      {status === 'success' && (
        <div className="flex items-center gap-2 p-4 rounded-xl mb-6 text-sm text-green-700" style={{ background: '#f0fdf4', border: '1px solid #bbf7d0' }}>
          <CheckCircle size={16} /> {message}{' '}
          <Link href="/admin" className="underline ml-1">Back to all trips →</Link>
        </div>
      )}
      {status === 'error' && (
        <div className="flex items-center gap-2 p-4 rounded-xl mb-6 text-sm text-red-700" style={{ background: '#fff1f2', border: '1px solid #fecaca' }}>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <Section title="Terms & Conditions">
          <textarea
            ...
            placeholder="e.g. All travelers must carry a valid government ID..."
            rows={5}
            className="w-full text-sm px-3 py-2 rounded-lg border outline-none resize-y focus:border-orange-400 transition-colors"
            style={{ borderColor: '#e5e7eb' }}
          />
